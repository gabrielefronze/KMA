...
class subprocessWrapper:
    def __init__(self, executableString, trigger=None, pollingInterval=1, runInBackground=False, customName=None):
        ...
        if self.runInBackground:
            if not customName:
                self.stdout = open(getLogDir()+self.args.replace('.','_').replace(' ','_').replace('/','_')+".satellite.log", "w")
                self.stderr = open(getLogDir()+self.args.replace('.','_').replace(' ','_').replace('/','_')+".satellite.err", "w")
            else:
